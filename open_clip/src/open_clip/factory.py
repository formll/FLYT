...
def create_model(
        ...
        downstream_logit_scale: Optional[float] = None,
        **model_kwargs,
):
    ...
    # Add FLYT downstream temperature
    if downstream_logit_scale is not None:
        model.downstream_logit_scale = nn.Parameter(torch.ones([]) * downstream_logit_scale)
        logging.info(f"Init Downstream logit scale: {model.downstream_logit_scale.item()}")
    ...
    if pretrained:
        ...
        if checkpoint_path:
            ...
            strict = downstream_logit_scale is None
            load_checkpoint(model, checkpoint_path, weights_only=load_weights_only, strict=strict)
        else:
            error_str = (
